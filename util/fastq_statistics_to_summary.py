...
#                os.system("ln -s %s %s/%s/%s" % (SampleFile, link_dir, sample_id, LinkFileName))
        ...
        if sample_count != 0:
            ...
            ResultFileList = []
            for flowcell_dir in list_flowcell:
                tmp_list = glob.glob("*/*%s*xls" %(sample_id))
                for _dir in tmp_list:
